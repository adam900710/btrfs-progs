...
int BOX_MAIN(mkfs)(int argc, char **argv)
{
	...
	while(1) {
		...
		static const struct option long_options[] = {
			...
			{ "data-size", required_argument, NULL, GETOPT_VAL_DATA_IO_SIZE },
#endif
	...
	*/
	...
	if (data_size) {
		...
		if (!is_power_of_2(data_size) || data_size <= sectorsize ||
			...
			error("data io size invalid, has %u expect power of 2 in range [%u, %u]",
			      data_size, sectorsize, BTRFS_MAX_METADATA_BLOCKSIZE);
			exit(1);
		}
		mkfs_cfg.data_size = data_size;
		features.compat_ro_flags |= BTRFS_FEATURE_COMPAT_RO_DATA_SIZE;
	}
